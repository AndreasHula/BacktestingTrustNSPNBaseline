...
//-------------------------------------------------------------------------------------------------------------------
	...
	ofstream ofs("/home/hulaa/work/Modelhold/DeGNSPNFullModNull0.bin", ofstream::out| ofstream::binary);			//output stream
	start_id= 0;
	end_id = 824;
	...
	for(int sub_id= start_id; sub_id < end_id; ++sub_id)
	{
		...
		for(int risk_count= 3; risk_count < 4; ++risk_count)
		{

			inv_risk_aversion = risk_grid[risk_count];
			tru_risk_aversion = risk_grid[risk_count];

			for(int temp_count= 1; temp_count < 2; ++temp_count)
			{
